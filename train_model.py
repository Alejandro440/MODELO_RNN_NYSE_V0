...
CLASS_LABELS: Sequence[str] = ["Muy Alcista", "Alcista", "Muy Bajista", "Bajista", "Lateral"]
...
def plot_confusion(y_true: np.ndarray, y_pred: np.ndarray, output_dir: Path) -> np.ndarray:
...
def write_report(
    ...
    history_dict = history.history
    with open(output_dir / "model_training_report.txt", "w", encoding="utf-8") as report_file:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Entrena el modelo RNN para predecir tendencias bursátiles.")
    parser.add_argument("--epochs", type=int, default=50, help="Número de épocas para el entrenamiento")
    parser.add_argument("--batch-size", type=int, default=128, help="Tamaño de batch")
    parser.add_argument("--n-steps", type=int, default=120, help="Número de pasos para las secuencias")
    parser.add_argument("--start-date", type=str, default="2002-01-01", help="Fecha inicial de los datos")
    parser.add_argument("--end-date", type=str, default="2024-01-01", help="Fecha final de los datos")
...
def main() -> None:
    ...
    history = model.fit(
        ...
        data.y_train,
        validation_data=([data.indices_test, data.X_test, data.X_sp500_test], data.y_test),
        epochs=args.epochs,
        batch_size=args.batch_size,
        callbacks=[early_stopping, reduce_lr],
    )

    test_loss, test_accuracy = model.evaluate([data.indices_test, data.X_test, data.X_sp500_test], data.y_test)
    print(f"Pérdida en Test: {test_loss}")
    print(f"Precisión en Test: {test_accuracy}")

    y_pred = model.predict([data.indices_test, data.X_test, data.X_sp500_test])
    y_pred_classes = np.argmax(y_pred, axis=1)

    plot_training_history(history, output_dir)
    conf_matrix = plot_confusion(data.y_test, y_pred_classes, output_dir)
    class_report = classification_report(data.y_test, y_pred_classes, target_names=CLASS_LABELS)

    write_report(output_dir, history, test_loss, test_accuracy, conf_matrix, class_report)

    model.save(output_dir / "my_model.h5")
    print(f"Modelo guardado: {output_dir / 'my_model.h5'}")
